...
func turnOnClimateControl(temp: Int) throws {
    if engineRunning == false {
        // Start engine
         try startEngine(gallonsGasInTank: 0)
    }
...
do {
    try turnOnClimateControl(temp: currentTemperature)
} catch RemoteError.outOfGas {
        print("out of gas")
} catch {
    print("error")
}
