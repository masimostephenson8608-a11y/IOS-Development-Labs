...
func buyItem(name: String) -> Double? {
    if let stocks = stock[name] {
        if stocks >= 1 {
            return prices[name]
        }
    }
...
if let item = buyItem(name: "Donuts") {
    print(item)
}
if let item = buyItem(name: "no") {
    print(item)
}
if let item = buyItem(name: "Chips") {
    print(item)
}
/*:
