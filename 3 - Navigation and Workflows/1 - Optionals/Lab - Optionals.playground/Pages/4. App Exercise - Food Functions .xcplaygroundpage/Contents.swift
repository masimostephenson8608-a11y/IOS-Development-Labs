...
if let food = foodLog(meal: "Lunch") {
    print(food)
}
/*:
...
func isItSaved() -> [String : Any]? {
    if let item = UserDefaults.standard.dictionary(forKey: "mealLog") {
        return item
    } else {
        return [:]
    }
